...
-- Select theme: picks for the currently active variant only, so an
-- asymmetric dark/light config (e.g. dark=tokyonight, light=catppuccin) is
-- preserved across single-side changes.
vim.keymap.set('n', '<leader>ct', function()
  local theme = require('config.theme')
  local cur = theme.current()
  vim.ui.select(theme.list(), {
    prompt = 'Theme for ' .. cur.variant .. ' mode',
    format_item = function(name)
      return name == cur[cur.variant] and (name .. '  (active)') or name
    end,
  }, function(choice)
    if not choice then
      return
    end
    theme.set({ [cur.variant] = choice })
  end)
