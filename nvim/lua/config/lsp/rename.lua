...
local function qf_rename()
  ...
  vim.ui.input({prompt = 'Rename To> ', default = position_params.oldName},
               function(input)
    if input == nil then
    ...
    vim.lsp.buf_request(0, "textDocument/rename", position_params,
      ...
      if not result or not result.changes then
        notify(string.format('could not perform rename'), 'error', {
          title = string.format('[lsp] rename: %s -> %s',
                                position_params.oldName, position_params.newName),
          timeout = 500
        })
        return
      end
      ...
      for uri, edits in pairs(result.changes) do
        ...
        for _, edit in ipairs(edits) do
          ...
          local line = vim.api.nvim_buf_get_lines(bufnr, start_line - 1,
                                                  start_line, false)[1]
        ...
        local short_uri =
            string.sub(vim.uri_to_fname(uri), #vim.fn.getcwd() + 2)
        notification = notification ..
                           string.format('made %d change(s) in %s', #edits,
                                         short_uri)
      end
