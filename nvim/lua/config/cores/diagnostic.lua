...
local severity_hl = {
  "DiagnosticError", "DiagnosticSignWarn", "DiagnosticInfo", "DiagnosticHint"
}
...
local function echo_cursor_diagnostic()
  ...
  if vim.tbl_isempty(cursor_diagnostics) then return end
  ...
  for _, diagnostic in ipairs(cursor_diagnostics) do
    ...
    local msg = (diagnostic.source or "") .. ": " ..
                    vim.fn
                        .trim(
                        vim.fn.substitute(diagnostic.message, "\n", "", "g")) ..
                    " "
...
local diag_au_id = vim.api.nvim_create_augroup("Cursor_Diagnostics",
                                               {clear = true})
