...
local function mode()
  ...
  local mode_color = string.format('Statusline%s',
                                   (mode_colors[current_mode] or 'Normal'))
...
local function file_name()
  ...
  return string.format('%s %%<%s %s', color('StatuslineAccent', '»'),
                       file_path, color('StatuslineAccent', '«'))
end

local function file_modified()
  if vim.bo.modified then return color('StatuslineBoolean', '+') end

  return nil
end

local function file_read_only()
  if vim.bo.readonly then return color('StatuslineBoolean', '‼') end
...
local function search_count()
  ...
  if vim.v.hlsearch > 0 and searchcount.total > 0 then
    return color('StatuslineSuccess',
                 string.format('%s/%s', searchcount.current, searchcount.total))
  end
...
local function lsp_status()
  ...
  if #errors ~= 0 then
    table.insert(messages,
                 color('StatuslineError', string.format('E%s', #errors)))
  end
  if #warnings ~= 0 then
    table.insert(messages,
                 color('StatuslineWarning', string.format('W%s', #warnings)))
  end
...
function _G.statusline()
  local sections = {
    mode(), icon(), '', file_name(), file_modified(), file_read_only(),
    -- Right side
    '%=', plugin_updates(), lsp_status(), ' '
  }

  return table.concat(vim.tbl_filter(function(section) return section end,
                                     sections), ' ')
end
