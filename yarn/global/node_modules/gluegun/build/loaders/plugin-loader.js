...
function loadPluginFromDirectory(directory, options) {
    ...
    if (jetpackPlugin.exists('commands') === 'dir') {
        var commands = jetpackPlugin.cwd('commands').find({ matching: commandFilePattern, recursive: true });
        plugin.commands = plugin.commands.concat(commands.map(function (file) { return command_loader_1.loadCommandFromFile(path.join(directory, 'commands', file)); }));
    }
    // load the extensions found in the extensions sub-directory
    if (jetpackPlugin.exists('extensions') === 'dir') {
        var extensions = jetpackPlugin.cwd('extensions').find({ matching: extensionFilePattern, recursive: false });
        plugin.extensions = extensions.map(function (file) { return extension_loader_1.loadExtensionFromFile(directory + "/extensions/" + file); });
    }
    else {
        plugin.extensions = [];
    }
    // load config using cosmiconfig
    var config = config_loader_1.loadConfig(plugin.name, directory);
