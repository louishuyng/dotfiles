...
# Bail gracefully if switchaudio-osx isn't installed.
# `brew install switchaudio-osx` to enable mic detection.
if ! command -v SwitchAudioSource >/dev/null 2>&1; then
  sketchybar -m --set mic label="no-switchaudio" icon= icon.color="$GREY" label.color="$GREY"
  exit 0
fi
